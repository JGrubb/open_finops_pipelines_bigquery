...
check-env: ## Validate required environment variables
	...
	@test -n "$(AWS_EXPORT_NAME)" || (echo "ERROR: AWS_EXPORT_NAME not set" && exit 1)
...
deploy-aws: check-env ## Deploy AWS billing pipeline
	...
	gcloud run jobs create aws-billing-pipeline \
		...
		--set-env-vars="SOURCES__AWS_CUR__BUCKET=$(AWS_GCS_BUCKET),SOURCES__AWS_CUR__PREFIX=$(AWS_GCS_PREFIX),SOURCES__AWS_CUR__EXPORT_NAME=$(AWS_EXPORT_NAME),SOURCES__AWS_CUR__TABLE_NAME=$(AWS_TABLE_NAME),SOURCES__AWS_CUR__PROJECT_ID=$(GCP_PROJECT_ID),SOURCES__AWS_CUR__DATASET=$(AWS_DATASET),DESTINATION__BIGQUERY__PROJECT_ID=$(GCP_PROJECT_ID),DESTINATION__BIGQUERY__LOCATION=US" \
		--command="uv" \
